...
> **提示：** 如果你克隆了仓库并以本地 MCP 服务器方式运行（如 `command: "node", args: ["build/index.js"]`），`commands/` 目录已就绪——直接输入 slash 命令即可。如果你用 `npx` 运行服务器，需将 `commands/` 文件夹复制到你的项目根目录或工作区，Claude Code 才能发现这些命令。
